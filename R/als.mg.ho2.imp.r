als.mg.ho2.imp <- function (Z, rawz0, W01, W02, A0, W1, W2, A, V, I, PHT, nvar, nlv1, nlv, ng, missingvalue, itmax, ceps)
{
	#---------------------------------------------------
	# ALS algorithm for GSCA with 2nd-order latents (multiple group analysis)
	...
	while (it <= itmax && imp > ceps) {
		...
		for (g in 1:ng) {
			...
			for (j in k:kk) {
				...
				W1[windex1,j] <- theta
				V[i:ii,t:tt] <- cbind(diag(1,nvar),W1[i:ii,k:kk]%*%W2[k:kk,l:ll])
			}
		}
		...
		# step 4: update missing observations
		W <- W1%*%W2
		Q <- V%*%I - W%*%A   
		for (j in 1:sizez) {
			zindex <- which(rawz0[,j] == missingvalue)
			if ( nrow(as.matrix(zindex)) != 0 ) {
				z <- Z[,j,drop=FALSE]
				q <- Q[j,,drop=FALSE]
				H6 <- diag(1, sizez)
				H6[j,j] <- 0
	...
	output.als.mg.ho2.imp <- list(W = W, W1 = W1, W2 = W2, A = A, Z = Z, Psi = Psi, Gamma = Gamma, f = f, it = it, imp = imp)
	output.als.mg.ho2.imp
}